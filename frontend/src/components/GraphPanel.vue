...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Transitioning from simulating to non-simulating: show hint
...
const renderGraph = () => {
  ...
  // Track index of current edge between each node pair
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    // Compute curvature: spread out for multiple edges, straight for single edge
    let curvature = 0
    if (totalCount > 1) {
  ...
  const getLinkPath = (d) => {
    ...
    // Detect self-loops
    if (d.isSelfLoop) {
      // Self-loop: draw an arc from node that returns to itself
      const loopRadius = 30
      // Start from node right side, loop back
      const x1 = sx + 8  // Start offset
      const y1 = sy - 4
      const x2 = sx + 8  // End offset
      const y2 = sy + 4
      // Draw self-loop as arc (sweep-flag=1 clockwise)
    ...
    // Perpendicular offset based on distance ratio, ensures visible curve
  ...
  // Link labels background (white background for better readability)
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        ...
        if (!d._isDragging && distance > 3) {
          // Only restart simulation on first true drag detection
          d._isDragging = true
