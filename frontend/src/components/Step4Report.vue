            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Complete (chapter generation done) -->
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract core entities - full extraction, including summaries and fact counts
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    const activeMatch = text.match(/(?:当前有效事实|Active Facts):\s*(\d+)/)
    ...
    const activeSection = text.match(/### (?:【当前有效事实】|Active Facts)[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
  ...
  try {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        if (!headerMatch) {
          headerMatch = line.match(/^-\s*(?:选择)?([^（(\s][^（(]*)(?:[（(]index\s*=?\s*\d+[)）])?[：:]\s*(.*)/)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
        ...
        if (name) {
          ...
          // Continuation of rationale (exclude ending summary paragraphs)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          ...
          if (interview.redditAnswer && interview.redditAnswer !== '（该平台未获得回复）' && interview.redditAnswer !== '(No response on this platform)') {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && interview.twitterAnswer !== '（该平台未获得回复）' && interview.twitterAnswer !== '(No response on this platform)') {
            interview.redditAnswer = interview.twitterAnswer
          }
        } else if (!twitterMatch && !redditMatch) {
          // No platform markers (very old format): use full text as answer
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/(?:找到|Found)\s*(\d+)\s*(?:条|results?)/)
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Active Key Facts (${props.result.facts.length})`)
        ]),
        h('button', {
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Drifting Query Analysis Sub-Questions'),
        ...
        // Empty state
        activeTab.value === 'facts' && props.result.facts.length === 0 && h('div', { class: 'empty-state' }, 'No active key facts'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '（该平台未获得回复）' || t === '(该平台未获得回复)' || t === '[无回复]' || t === '(No response on this platform)'
    }
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform toggle button (shown only when real dual-platform answers exist)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading level-2 heading (## xxx) since section title is shown in outer container
  ...
  // Handle line breaks - empty lines become paragraph separators, single breaks become <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Clean <p><br> followed by block elements (caused by excess blank lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Clean consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Clean <br> before paragraph starts following block elements
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // Scroll logic handled in nextTick after loop completes
          }
...
const extractFinalContent = (response) => {
  ...
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Remove Thought: prefixed thought process
